...
export const serializeParams = ({ page, filters }: UseCustomSearchParamsType) =>
  Object.entries({
    page: String(page),
    ...(filters || {}),
...
export const deserializeParams = (params: URLSearchParams) => {
  ...
  params.forEach((key, value) => {
    paramsArray.push([value, key]);
  });
  const entriesToMapObj = new Map(paramsArray);
  return Object.fromEntries(entriesToMapObj);
};
